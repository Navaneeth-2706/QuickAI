...
const GenerateImages = () => {
  const Imagetype= ['Realistic' , 'Gibli style', 'Anime style', 'Cartoon style', 
    ...
    const onSubmitHandler = async(e)=>{
      ...
      try {
        ...
        const prompt = `Generate an image of ${input} in the style ${selectedtype}`
    ...
    return (
      <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
            ...
            <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
                ...
                {Imagetype.map((item,index)=>(
                  <span onClick={()=>setSelectedtype(item)}
                   className={`text-xs px-4 py-1 border rounded-full cursor-pointer 
                    ${selectedtype === item? 'bg-green-50 text-green-700':
              ...
              <br/>
              <button disabled={loading} className='w-full flex justify-center items-center gap-3 bg-gradient-to-r from-[#32CD32] to-[#228B22] 
              text-white px-4 py-2 mt-4 text-sm rounded-lg transition cursor-pointer'>
            ...
            {/* right col */}
            <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-94 '>
              <div className='flex items-center gap-3'>
                <Image className='w-5 h-5 text-[#32CD32]'/>
